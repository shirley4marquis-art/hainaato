...
export async function reserveDocumentNumber(client: PoolClient, type: DocumentType, year = new Date().getUTCFullYear()): Promise<string> {
  ...
  const prefix = `${type === "quotation" ? "NINDGE-AUTO" : "HA"}-${PREFIX[type]}-${year}-`;
  const legacy = type === "quotation" ? Number((await client.query("SELECT COALESCE(MAX(substring(document_number from '[0-9]+$')::bigint),0) AS n FROM quotes WHERE document_number ~ $1", [`^(HA|NINDGE-AUTO)-QT-${year}-[0-9]+$`])).rows[0].n) : 0;
