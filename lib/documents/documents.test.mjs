...
test('safe migration, private tables, sequential concurrent numbering and rollback',async()=>{
 ...
 try{
  ...
  assert.equal(new Set(numbers).size,20);assert.equal(numbers[0],'NINDGE-AUTO-QT-2026-0100');assert.equal(numbers.at(-1),'NINDGE-AUTO-QT-2026-0119');
  await assert.rejects(db.transaction(async tx=>{await reserveDocumentNumber(tx,'quotation',2026);throw Error('rollback');}));
  assert.equal(await db.transaction(tx=>reserveDocumentNumber(tx,'quotation',2026)),'NINDGE-AUTO-QT-2026-0120');
...
test('long transmission values use an unsigned appendix before the signed final page',async()=>{
 const source=await PDFDocument.create();source.addPage([595,842]);const signed=source.addPage([595,842]);signed.drawText('AUTHORIZED SIGNATURE',{x:40,y:100});
 const bytes=await source.save(),before=hash(bytes);
 const transmission='Electronically controlled continuously variable transmission (e-CVT), with manual sequential mode and all-wheel drive';
 const field={...defaultField(),field:'transmission',x:424.75,y:349.2,width:122,height:15,maxLines:1,fontSize:10,minFontSize:7};
 const mapping={...structuredClone(EMPTY_MAPPING),reviewed:true,lockedPages:[2],fields:[field,{...field,id:"repeated-transmission",y:400}]};
 for(const language of ['en','es','zh','es-zh']){
  const output=await generatePdf(bytes,mapping,{values:{transmission,vehicle_brand:'Toyota',vehicle_model:'RAV4',document_number:'TEST-001'},vehicles:[],images:[],language});
  const pdf=await PDFDocument.load(output);assert.equal(pdf.getPageCount(),3);
  assert.equal(hash(bytes),before,'Source bytes are immutable');
  const streams=(doc,page)=>page.node.Contents().asArray().map(ref=>hash(doc.context.lookup(ref).contents));
  const original=await PDFDocument.load(bytes);
  assert.deepEqual(streams(pdf,pdf.getPage(2)),streams(original,original.getPage(1)),'Signed page stays unchanged and last');
  const {getDocument}=await import('pdfjs-dist/legacy/build/pdf.mjs');
  const task=getDocument({data:new Uint8Array(output),useSystemFonts:true}),rendered=await task.promise;
  const cover=(await (await rendered.getPage(1)).getTextContent()).items.map(i=>i.str??'').join(' ');
  const appendix=(await (await rendered.getPage(2)).getTextContent()).items.map(i=>i.str??'').join(' ').replace(/\s+/g,' ');
  assert.match(cover,/T1/);assert.ok(appendix.includes(transmission));assert.ok(appendix.includes('Toyota'));await task.destroy();
 }
});

test('short transmissions remain inline and explicit overflow mappings remain in control',async()=>{
 const pdf=await PDFDocument.create();pdf.addPage([595,842]);const bytes=await pdf.save();
 const field={...defaultField(),field:'transmission',x:40,y:40,width:122,height:15,maxLines:1,fontSize:10,minFontSize:7};
 const mapping={...structuredClone(EMPTY_MAPPING),reviewed:true,fields:[field]};
 assert.equal((await PDFDocument.load(await generatePdf(bytes,mapping,{...data,values:{transmission:'Automatic'}}))).getPageCount(),1);
 const custom={...mapping,fields:[{...field,overflow:{page:1,x:40,y:100,width:400,height:600,insertBefore:2}}]};
 assert.ok((await PDFDocument.load(await generatePdf(bytes,custom,{...data,values:{transmission:'Automatic transmission '.repeat(40)}}))).getPageCount()>1);
});
