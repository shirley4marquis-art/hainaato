...
test("customer email sends a valid Resend attachment payload", async () => {
  ...
  globalThis.fetch = async (_url, init) => {
    ...
    return new Response(JSON.stringify({ id: "re_test_message" }), { status: 200, headers: { "content-type": "application/json" } });
  };
  try {
    const { customSalesEmailHtml, sendEmail } = load("lib/email.ts");
    const html = customSalesEmailHtml({ customerName: "Buyer", heading: "Your quotation", message: "Ready" });
    const result = await sendEmail({ to: "buyer@real-domain.com", subject: "Your quotation", html });
    assert.deepEqual(result, { ok: true, providerMessageId: "re_test_message" });
    assert.deepEqual(payload.attachments, []);
    assert.match(payload.html, /https:\/\/www\.nindgeauto\.com\/nindge-mark\.png/);
    assert.doesNotMatch(JSON.stringify(payload), /"path"/);
  } finally {
