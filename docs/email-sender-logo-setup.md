...
- Domain: `hainaautochina.com`
- Current DMARC: `v=DMARC1; p=none`
- Current BIMI: no `default._bimi.hainaautochina.com` TXT record found
...
Add or update DMARC after SPF and DKIM are correctly aligned for every service that sends mail as `@hainaautochina.com`, including Hostinger and Resend.
...
Host: _dmarc
Type: TXT
Value: v=DMARC1; p=quarantine; rua=mailto:sales@hainaautochina.com; adkim=s; aspf=s; pct=100
```
...
Host: _dmarc
Type: TXT
Value: v=DMARC1; p=reject; rua=mailto:sales@hainaautochina.com; adkim=s; aspf=s; pct=100
```
...
Host: default._bimi
Type: TXT
Value: v=BIMI1; l=https://www.hainaautochina.com/bimi-logo.svg; a=
```
...
Host: default._bimi
Type: TXT
Value: v=BIMI1; l=https://www.hainaautochina.com/bimi-logo.svg; a=https://www.hainaautochina.com/vmc.pem
```

## Provider Account Steps

1. Confirm Resend is authenticating the exact sender domain with DKIM records for `hainaautochina.com`.
2. Confirm Hostinger mail has DKIM enabled for `hainaautochina.com`.
3. Set the mailbox profile image for `sales@hainaautochina.com` inside Hostinger or Google Workspace if the mailbox is connected there.
