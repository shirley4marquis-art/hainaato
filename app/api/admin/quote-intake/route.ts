...
function slugFromVehicleUrl(value: string): string | null {
  try {
    const url = new URL(value, "https://www.hainaautochina.com");
    if (!new Set(["www.hainaautochina.com", "hainaautochina.com", "localhost"]).has(url.hostname)) return null;
...
export async function POST(request: Request) {
  ...
  const vehicles = urls.map((sourceUrl) => {
    ...
    return {
      sourceUrl: `https://www.hainaautochina.com/vehicles/${encodeURIComponent(vehicle.slug)}`,
      item: {
