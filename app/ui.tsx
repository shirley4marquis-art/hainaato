...
export function SiteShell({children}:{children:ReactNode}) {
  ...
  return <>
    ...
    <header className="ah-header"><div className="container">
      <div className="mobile-header-minimal"><div className="mobile-header-brand"><Link href="/" aria-label="Nindge Automobile home"><img src="/images/KkwGH.png" alt=""/><span>NINDGE<small>AUTOMOBILE</small></span></Link></div><form className="mobile-header-search" action="/vehicles" method="get" role="search"><input name="q" type="search" list="mobile-vehicle-search-suggestions" placeholder="Search vehicle e.g. JAC" aria-label="Search vehicle"/><datalist id="mobile-vehicle-search-suggestions"><option value="JAC"/><option value="Toyota"/><option value="BYD"/><option value="Chery"/><option value="Ford"/></datalist><button type="submit" aria-label="Search vehicles"><Search aria-hidden="true"/></button></form><Link className="ah-cart-link" href="/cart" aria-label={`Cart, ${cartCount} vehicle${cartCount===1?"":"s"}`}><ShoppingCart/>{cartCount>0&&<span className="ah-cart-badge">{cartCount}</span>}</Link><Link className="mobile-header-quote" href="/quote">Get Quote</Link></div>
      ...
      <div className="ah-header-brand-row">
        <Link className="ah-brand-lockup" href="/"><img src="/images/KkwGH.png" alt="Nindge Automobile" className="ah-header-brand-logo"/><div className="ah-brand-copy"><div className="ah-brand-headline"><span className="ah-brand-title">NINDGE AUTOMOBILE</span><span className="ah-brand-ribbon">China auto export</span></div><span className="ah-brand-lead">Used &amp; new vehicles from China — transparent condition, export-ready docs, worldwide delivery.</span><span className="ah-brand-tagline">◎ <span>From China, to the world</span></span></div></Link>
        <div className="ah-header-aside"><img src="/images/KkwGH.png" alt="Nindge Automobile" className="ah-header-corner-logo"/><div className="ah-header-actions"><Link className="ah-cart-link" href="/cart" aria-label={`Cart, ${cartCount} vehicle${cartCount===1?"":"s"}`}><ShoppingCart/>Cart{cartCount>0&&<span className="ah-cart-badge">{cartCount}</span>}</Link><Link className="ah-btn-gold" href="/quote">Request Quote</Link><Link className="ah-btn-outline" href="/contact">Contact</Link></div><div className="ah-header-meta"><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp at +8615032178759"><WhatsAppIcon/> +8615032178759</a><a href="mailto:info@nindgeauto.com">✉ info@nindgeauto.com</a><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"><WhatsAppIcon/>WhatsApp</a><a href={TELEGRAM_URL} target="_blank" rel="noopener noreferrer"><TelegramIcon/>Telegram</a><a href={WECHAT_CONTACT_URL}><WeChatIcon/>WeChat</a></div></div>
      </div>
      <div className="ah-header-nav-wrap"><button className="menu" onClick={()=>setOpen(!open)} aria-expanded={open} aria-label="Toggle navigation">{open?"×":"☰"}</button><nav className={open?"open":""}><div className="ah-nav-links">{nav.map(({test,href,label,zh,Icon,children})=>{const active=test.test(path);return <div className={`ah-nav-item${children?" has-submenu":""}`} key={href}>{children?<button type="button" className={active?"active":""} onClick={()=>setVehiclesOpen(value=>!value)} aria-expanded={vehiclesOpen}><Icon/><span>{label}<small lang="zh-CN" translate="no">{zh}</small></span><ChevronDown className={`menu-chevron${vehiclesOpen?" open":""}`}/></button>:<Link className={active?"active":""} aria-current={active?"page":undefined} href={href} onClick={()=>setOpen(false)}><Icon/><span>{label}<small lang="zh-CN" translate="no">{zh}</small></span></Link>}{children&&<div className={`ah-nav-submenu${vehiclesOpen?" open":""}`} aria-label="Vehicle categories">{children.map(child=>{const childActive=child.test.test(path);return <Link className={childActive?"active":""} aria-current={childActive?"page":undefined} href={child.href} key={child.href} onClick={()=>{setOpen(false);setVehiclesOpen(false)}}><child.Icon/><span>{child.label}<small lang="zh-CN" translate="no">{child.zh}</small></span></Link>})}</div>}</div>})}</div><div className="ah-nav-end"><form className="ah-header-search" action="/vehicles" method="get" role="search"><input type="search" name="q" placeholder="Quick search..." aria-label="Search vehicles"/><button type="submit" aria-label="Search vehicles"><Search size={16} aria-hidden="true"/><span>Search</span></button></form></div></nav></div>
    </div>
    <ShipmentTicker/>
    </header>
...
function Footer(){return <footer className="footer"><div className="container footer-grid"><div className="footer-about"><div className="brand light"><img src="/images/KkwGH.png" alt=""/><span>NINDGE<b>AUTOMOBILE</b></span></div><p>Professional Chinese parallel auto exporter serving global buyers with carefully inspected vehicles and end-to-end export services.</p><section className="footer-company-cn" aria-label="公司简介" lang="zh-CN" translate="no"><p>Nindge Automobile 专注于中国新车与二手车出口，为全球客户提供车辆采购、专业验车、出口文件及国际物流支持。</p></section><div className="social"><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"><WhatsAppIcon/>WhatsApp</a><a href={TELEGRAM_URL} target="_blank" rel="noopener noreferrer"><TelegramIcon/>Telegram</a><a href={WECHAT_CONTACT_URL}><WeChatIcon/>WeChat</a></div></div><div><h4>User Guide</h4><Link href="/help">FAQ (Help)</Link><Link href="/export-process">Purchase Process</Link><Link href="/vehicles">Vehicle Market</Link><Link href="/news">Industry News</Link></div><div><h4>Export Services</h4><Link href="/services">Logistics Solutions</Link><Link href="/services">Customs Clearance</Link><Link href="/services">Refurbishment</Link><Link href="/services">Overseas Warehousing</Link></div><div><h4>Contact Us</h4><p>Contact Us (24/7)<br/><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer">+86 150 3217 8759</a></p><p>Business Email<br/><a href="mailto:info@nindgeauto.com">info@nindgeauto.com</a></p><p>Fujian Province, China</p></div></div><div className="container footer-bottom"><span>© 2026 nindgeauto.com · China Used Cars Export</span><span><Link href="/privacy">Privacy Policy</Link> · <Link href="/terms">Terms of Service</Link> · <Link href="/cookie">Cookie Policy</Link> · <Link href="/disclaimer">Disclaimer</Link></span></div></footer>}
...
export function VehicleCard({v}:{v:VehicleIndexEntry}){const specLine=[v.year,v.mileageKm!=null?formatKm(v.mileageKm):null,fuelChoiceLabel(v.fuel)].filter(Boolean).join(" · ");const candidates=(v.thumbs.length?v.thumbs:v.thumb?[v.thumb]:[]).map(img=>imagePath(v.site,v.id,img));const sourceLabel="Nindge Automobile";return <article className="car reveal"><Link href={`/vehicles/${v.slug}`}><div className="car-image"><ResilientVehicleImage candidates={candidates} alt={`${v.title} exterior`}/><span>{v.availability === "sold" ? <QuoteCopy en="Sold out" es="Agotado"/> : sourceLabel}</span></div></Link><div className="car-body"><small>{v.bodyType?v.bodyType.toUpperCase():"EXPORT AVAILABLE"}</small><Link href={`/vehicles/${v.slug}`}><h3>{v.title}</h3></Link><p>{specLine||v.location||"Export ready"}</p><p className="cif-card-note">CIF included: freight + insurance</p><div><b><Price cny={v.priceCNY}/></b></div><div className="vehicle-browse-actions"><Link className="vlist-view" href={`/vehicles/${v.slug}`}><QuoteCopy en="View vehicle" es="Ver vehículo"/></Link>{v.availability !== "sold" && <Link className="vlist-inquire" href={`/quote?vehicle=${encodeURIComponent(v.slug)}`}><QuoteCopy en="Request a quote" es="Solicitar cotización"/></Link>}</div></div></article>}
