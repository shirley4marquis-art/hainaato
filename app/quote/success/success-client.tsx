...
export function QuoteSuccessClient({quoteRef,token}:{quoteRef:string;token:string}){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{try{const response=await fetch(`/api/quote-success?ref=${encodeURIComponent(quoteRef)}&token=${encodeURIComponent(token)}`,{cache:"no-store"});const data=await response.json();if(!response.ok)throw new Error(data.error||"Could not load this quotation.");if(cancelled)return;setQuote(data.quote);setError(null);attempts+=1;if(data.quote.deliveryStatus==="processing"){if(attempts<22)timer=setTimeout(load,15000);else setPollingExpired(true)}}catch(cause){if(!cancelled)setError(cause instanceof Error?cause.message:"Could not load this quotation.")}};
  ...
  const deliveryCopy=quote.deliveryStatus==="sent"?"Your quotation email has been submitted for delivery. Please check your inbox and spam folder.":quote.deliveryStatus==="failed"?"We could not send your quotation email. Try downloading the PDF below or contact our sales team.":quote.deliveryStatus==="whatsapp-only"?"No email was supplied. Download the PDF or continue on WhatsApp.":pollingExpired?"Email preparation is taking longer than expected. Refresh to check again, download the PDF, or contact our sales team.":"Your quote is saved. The PDF email is being prepared in the background.";
