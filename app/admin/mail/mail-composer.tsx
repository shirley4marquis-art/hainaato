...
export function MailComposer({ clients, initialCustomerId }: { clients: AdminCustomerSummary[]; initialCustomerId?: number }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else { setNotice(`Email sent to ${to} from NINDGE AUTOMOBILE | 海纳百川国际汽贸.`); setSubject(""); setHeading(""); setMessage(""); setCtaLabel(""); setCtaUrl(""); }
    } catch { setError("Network error — please try again."); }
    finally { setBusy(false); }
  }

  return <form className={styles.mailComposer} onSubmit={submit}>
    <div className={styles.mailComposerHead}><div><span>FROM</span><b>NINDGE AUTOMOBILE | 海纳百川国际汽贸</b></div><Send size={18}/></div>
    <div className={styles.mailFields}>
      ...
      <label className={styles.mailWide}>Subject<input value={subject} onChange={(event) => setSubject(event.target.value)} required maxLength={180} placeholder="Your Nindge Automobile order update" /></label>
    ...
    <div className={styles.mailPreviewNote}><Eye size={13}/><span>Your text is automatically placed inside the complete Nindge Automobile branded email design with greeting, signature and contact footer.</span></div>
