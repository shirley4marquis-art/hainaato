...
export function VehicleRequestForm({vehicleSlug,vehicleTitle}:{vehicleSlug:string;vehicleTitle:string}){
  ...
  return <form id="quote-form" className="request-form compact" onSubmit={submit} aria-busy={state==="sending"}><h2>Get an Instant Vehicle Quote</h2><p style={{margin:"-8px 0 16px",fontSize:13,color:"var(--muted)"}}>Enter your delivery details and we will create your quotation PDF and email it to you immediately.</p><div className="form-grid"><label htmlFor="rv-vehicle">Vehicle</label><input id="rv-vehicle" name="vehicle" defaultValue={vehicleTitle} readOnly/><label htmlFor="rv-name">Full name *</label><input id="rv-name" name="name" required autoComplete="name" autoFocus/><label htmlFor="rv-email">Email *</label><input id="rv-email" name="email" type="email" required autoComplete="email"/><label htmlFor="rv-whatsapp">Phone / WhatsApp *</label><input id="rv-whatsapp" name="whatsapp" type="tel" placeholder="+58..." required autoComplete="tel"/><label htmlFor="rv-destination">Destination country *</label><input id="rv-destination" name="destination" placeholder="e.g. Venezuela" required autoComplete="country-name"/><label htmlFor="rv-cityState">City / State</label><input id="rv-cityState" name="cityState" autoComplete="address-level2"/><label htmlFor="rv-destinationPort">Destination port</label><input id="rv-destinationPort" name="destinationPort" placeholder="e.g. Puerto Cabello"/><label htmlFor="rv-quantity">Quantity</label><input id="rv-quantity" name="quantity" type="number" min={1} max={50} defaultValue={1}/><label className="wide" htmlFor="rv-message">Additional requirements</label><textarea className="wide" id="rv-message" name="message" rows={3} placeholder="Color, timeline, incoterms…"/><label className="wide consent-checkbox"><input type="checkbox" name="publicConsent" id="rv-consent"/> {CONSENT_LABEL}</label></div><button className="btn primary" disabled={state==="sending"}>{state==="sending"?"Generating quotation & PDF…":"Email My Quote & PDF"}</button><div role="status" aria-live="polite">{state==="sent"&&(emailSent?<p className="success">Your quotation and PDF have been sent to {submittedEmail}{ref?` (reference ${ref})`:""}.</p>:<p className="form-error">Your quotation was created{ref?` (reference ${ref})`:""}, but the email could not be delivered. Our team has the request and will resend it.</p>)}{state==="error"&&<p className="form-error" role="alert">{error}</p>}</div></form>
}
